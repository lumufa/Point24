...
<p align="center">
  <img src="docs/hero.png" width="640" alt="Point24 — 经典模式主界面" />
</p>
...
- [玩法 + 三种模式](#玩法--三种模式)
- [立即体验](#立即体验)
- [技术架构](#技术架构核心逻辑-0-平台依赖)
- [技术亮点](#技术亮点)
  - [1. DFS 回溯求解器 + 自定义 Fraction 分数类（核心）](#1-dfs-回溯求解器--自定义-fraction-分数类核心)
  - [2. 哈希种子驱动的"每日挑战"——服务端零参与](#2-哈希种子驱动的每日挑战服务端零参与)
  - [3. 抖音平台适配层 + 激励视频降级](#3-抖音平台适配层--激励视频降级)
- [项目结构](#项目结构)
- [本地开发](#本地开发)
- [参与贡献](#参与贡献)
- [已知问题](#已知问题)
...
Point24 想证明：这两件事在客户端就能干净地做掉，而且代码量不大。整个项目核心算法 + 状态机 + 抖音适配，不到 30 个 TS 文件。
...
| 经典模式 | 限时冲刺 | 每日挑战 |
|:---:|:---:|:---:|
| <img src="docs/hero.png" width="300" alt="经典模式" /> | <img src="docs/sprint.png" width="300" alt="90 秒倒计时" /> | <img src="docs/daily.png" width="300" alt="每日同题" /> |

---

## 立即体验

> 当前状态：**单场景搭建完成，未上架**。详见 [已知问题](#已知问题)。

| 平台 | 当前可用方式 | 说明 |
|---|---|---|
| **抖音小游戏** | 暂未在抖音搜索中可用 | 未提交字节跳动开发者后台审核；上架后关键词将是「算24点」「数24点」 |
| **本地预览（浏览器）** | Cocos Creator 3.8.8 打开本目录 → 预览到浏览器 | `isWeb()` 分支生效，存储走 `localStorage`，激励视频自动降级为"无 SDK"分支（参见 [亮点 3](#3-抖音平台适配层--激励视频降级)） |

> 没有服务端依赖——每日挑战的题目由本地 PRNG + 哈希种子生成（[亮点 2](#2-哈希种子驱动的每日挑战服务端零参与)），离线就能跑。
...
下图是 Cocos Creator 编辑器里的场景结构：`GameStage` 节点持有 `Card1-4` / `OperatorPad` / `ModePad` / 状态文本等所有 UI 节点的引用（右侧 Inspector），状态机层把 reducer 输出的 state 写到这些节点上——UI 是 state 的纯函数，组件本身不持有领域状态。

<p align="center">
  <img src="docs/editor.png" width="720" alt="Cocos Creator 场景结构 + GameStage 组件绑定" />
</p>
...
│   │   │                           # 输入 action、输出 state + effects，引擎不感知
│   │   ├── platform/               # 抖音 tt.* 适配层 + 浏览器 / 内存兜底三套实现
│   │   │                           # tt-env / tt-storage / tt-rewarded-ad / tt-share / tt-audio / tt-banner
│   │   ├── ui/                     # Cocos 组件（GameStage / CardView / OperatorPad / ModePad / theme）
│   │   └── audio/                  # SoundManager
│   └── scenes/                     # Cocos 场景（GameStage 主场景）
└── package.json                    # Cocos Creator 3.8.8 工程描述
```

依赖方向硬约束：`ui → state → core`、`platform` 自成一层、`core` 不 import 任何 `cc.*` 或 `tt.*`（详见 `assets/scripts/README.md:11`）。
...
2. 直接预览到浏览器——`isWeb()` 分支生效，存储走 `localStorage`，激励视频自动走 `'no-sdk'` 分支
3. 构建抖音小游戏：菜单 → 项目 → 构建发布 → 平台选 `bytedance-mini-game`
4. 跑核心算法单测（可选）：因 `core/` 不依赖 Cocos，可在 Node 里直接 `npm test` 或 `vitest` 单跑 `core/*.ts`
...
- **大改动先开 issue 对齐**：涉及核心算法（求解器 / Fraction / 每日种子）、平台适配契约（`platform/` 接口签名）、状态机契约（`GameStore` action / state 形状）的 PR 必须先在 issue 中讨论。
- **分支命名**：`feat-<scope>` / `fix-<scope>` / `docs-<scope>` / `refactor-<scope>`，例如 `feat-tournament-mode`、`fix-rewarded-ad-cancel`。
- **Commit 规范**：遵循 [Conventional Commits](https://www.conventionalcommits.org/zh-hans/v1.0.0/)（`feat:` / `fix:` / `docs:` / `chore:` / `refactor:` 等前缀）。
- **合并方式**：所有 PR 走 **Squash merge**，保持主分支线性。
- **跨层禁忌**：`core/` 和 `state/` **绝对不能 import 任何 `cc.*` 或 `tt.*`**——这是项目的硬约束（[技术架构](#技术架构核心逻辑-0-平台依赖)），违反这条会让"核心逻辑跨环境跑"的能力直接破产。审查时会看 `import` 列表。
- **测试要求**：动到 `core/solver.ts` / `core/fraction.ts` / `core/seed.ts` 的纯函数必须同步加单元测试用例（可在 Node 里跑，不依赖编辑器）。

---

## 已知问题

- [ ] **抖音小游戏未上架**：尚未提交字节跳动开发者后台审核；目前只能本地通过 Cocos Creator 预览（浏览器分支）或自构建后用抖音开发者工具调试。
- [ ] **目前只有一个主场景**：`GameStage` 一个主场景承载经典 / 限时冲刺 / 每日挑战三种模式，未做独立场景拆分；多人对战 / 房间制场景尚未规划。
- [ ] **每日挑战可被改本地时钟提前看题**：种子算法纯客户端（[亮点 2](#2-哈希种子驱动的每日挑战服务端零参与)）—— 选种子方案就是接受这点代价，待业务方向转向竞技再改为服务端推题（替换点见 `deal.ts:103`）。
- [ ] **核心算法单测尚未完整**：`core/solver.ts` / `core/fraction.ts` / `core/seed.ts` 的纯函数虽然可在 Node 里测，但当前仓库未配置 `vitest` / `jest` 入口；正式上架前需补齐。
- [ ] **题库覆盖未做完整 enumerate**：进阶题池（`constants.ts:20`）目前是手挑的，理论上可以 enumerate 全部 1820 种四张组合并按求解器解的数量分级，目前没做。
- [ ] **激励视频文案 / 节流策略尚硬编码**：免费一次 + 看广告续一次的策略写死在 `GameStore.useHint()`（`GameStore.ts:201`），未抽出可配。